...
start_backend() {
    ...
    if [ -d "$backend_dir" ] && [ -f "$backend_dir/package.json" ]; then
        echo "▶️  Starting backend/GraphQL (port $BACKEND_PORT)..."
        cd "$backend_dir"
...
start_server() {
    ...
    echo "  GraphQL   : http://localhost:${BACKEND_PORT:-4000}/graphql"
}
...
show_status() {
    [ -f "$ENV_FILE" ] && source "$ENV_FILE"
    BACKEND_PORT="${BACKEND_PORT:-4000}"; FRONTEND_PORT="${FRONTEND_PORT:-3000}"
    echo ""; echo "📊 Social Media Feed Status:"; echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    fuser "${BACKEND_PORT}/tcp" &>/dev/null 2>&1 && echo "  ✅ Backend/GraphQL (port $BACKEND_PORT): RUNNING" || echo "  ❌ Backend/GraphQL (port $BACKEND_PORT): STOPPED"
    ...
    echo "  GraphQL   : http://localhost:$BACKEND_PORT/graphql"; echo ""
}

edit_ports() {
    [ -f "$ENV_FILE" ] && source "$ENV_FILE"
    BACKEND_PORT="${BACKEND_PORT:-4000}"; FRONTEND_PORT="${FRONTEND_PORT:-3000}"
    echo ""; echo "⚙️  Edit Ports - Social Media Feed"; echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    cat > "$ENV_FILE" <<EOF
# Social Media Feed - Port Configuration
