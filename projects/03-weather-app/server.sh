...
show_status() {
    ...
    echo ""
    echo "🔑 API Key:"
    if [ -f "$ENV_FILE" ] && grep -q "WEATHER_API_KEY\|OPENWEATHER" "$ENV_FILE"; then
        echo "  ✅ .env found with API key"
    else
        echo "  ⚠️  Set VITE_WEATHER_API_KEY in .env"
...
edit_ports() {
    [ -f "$ENV_FILE" ] && source "$ENV_FILE"
    DEV_PORT="${DEV_PORT:-3000}"
    local VITE_WEATHER_API_KEY="${VITE_WEATHER_API_KEY:-}"

    echo ""
    echo "⚙️  Edit Ports - Weather App"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    local existing_key="${VITE_WEATHER_API_KEY}"

    cat > "$ENV_FILE" <<EOF
# Weather App - Port Configuration
DEV_PORT=$DEV_PORT

# API Keys (do not change here — set your actual key below)
VITE_WEATHER_API_KEY=${existing_key}
EOF
    echo ""
