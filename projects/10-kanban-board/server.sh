...
# Server Management Script - Project 10: Kanban Board (Drag-Drop)
# Tech Stack: Express.js + Socket.IO + React | MongoDB | react-beautiful-dnd
...
show_menu() {
    echo ""
    echo "╔════════════════════════════════════════════════════════════╗"
    echo "║   Project 10: Kanban Board (Drag-Drop)                     ║"
    echo "║   Stack: Express + Socket.IO + React | MongoDB             ║"
    echo "╚════════════════════════════════════════════════════════════╝"
    echo ""
    ...
    echo "8.  Seed Database (Sample Board)"
...
install_deps() {
    ...
    if [ ! -f "$PROJECT_DIR/backend/package.json" ] && [ ! -f "$PROJECT_DIR/package.json" ]; then
        ...
        echo "   Backend: npm install express socket.io mongoose cors dotenv jsonwebtoken bcrypt"
        echo "   Frontend: npm install react socket.io-client axios @hello-pangea/dnd react-router-dom tailwindcss"
    fi
    echo "✅ Done"
}
...
start_mongodb() {
    ...
    if command -v docker &>/dev/null; then
        docker run -d --name kanban-mongo \
            -p "$MONGO_PORT:27017" mongo:latest 2>/dev/null || docker start kanban-mongo 2>/dev/null
        sleep 3
...
stop_mongodb() {
    ...
    command -v docker &>/dev/null && docker stop kanban-mongo 2>/dev/null
...
show_status() {
    echo ""
    echo "📊 Server Status - Kanban Board"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    if command -v docker &>/dev/null; then
        local st
        st=$(docker inspect -f '{{.State.Status}}' kanban-mongo 2>/dev/null || echo "not found")
        echo "  MongoDB  : $([ "$st" = "running" ] && echo "✅ Running (port $MONGO_PORT)" || echo "❌ $st")"
    fi
    echo ""
    echo "🌐 Access:"
    echo "  Kanban Board : http://localhost:$FRONTEND_PORT"
    echo "  API          : http://localhost:$BACKEND_PORT/api"
    echo "  WebSocket    : ws://localhost:$BACKEND_PORT (real-time sync)"
    echo ""
    echo "🔄 Real-time sync via Socket.IO — drag cards across columns!"
    echo ""
}

seed_database() {
    echo "🌱 Seeding Kanban board with sample data..."
    if [ -f "$PROJECT_DIR/backend/package.json" ] && grep -q '"seed"' "$PROJECT_DIR/backend/package.json"; then
        ...
        echo ""
        echo "   Sample board structure:"
        echo "   Board: 'My Project'"
        echo "   Columns: ['To Do', 'In Progress', 'Review', 'Done']"
        echo "   Cards: ['Design UI', 'Setup DB', 'Write API', 'Write Tests']"
    fi
}
